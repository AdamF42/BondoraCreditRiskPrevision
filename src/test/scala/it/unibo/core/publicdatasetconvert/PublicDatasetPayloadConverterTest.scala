...
class PublicDatasetPayloadConverterTest extends AnyWordSpec with Matchers {
  ...
  implicit val session = setupSparkSession
  ...
  val dfResult = PublicDatasetPayloadConverter.publicDStoDF(Seq(payloadInput))

  val dfExpected = session.read.format("csv")
    .option("header", true)
    .load("src/test/scala/it/unibo/core/publicdatasetconvert/test.csv")
